...
export default function HomeHero3D({ onBookClick }: HomeHero3DProps) {
  ...
  const scale = useTransform(scrollY, [0, 500], [1.5, 1.08]);
  ...
  // GSAP Entrance Animations (Keep only ambient particles animation)
  useEffect(() => {
    if (!isMounted) return;
  ...
  // Cloudinary Video URLs for Desktop vs Mobile
  const cloudName = process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || "demo";
  const desktopVideo = process.env.NEXT_PUBLIC_CLOUDINARY_HERO_DESKTOP_VIDEO || "samples/sea-turtle";
  const mobileVideo = process.env.NEXT_PUBLIC_CLOUDINARY_HERO_MOBILE_VIDEO || "samples/sea-turtle";
  const currentVideoPublicId = isMobile ? mobileVideo : desktopVideo;

  const videoUrl = `https://res.cloudinary.com/${cloudName}/video/upload/f_auto,q_auto/${currentVideoPublicId}.mp4`;
  const posterUrl = `https://res.cloudinary.com/${cloudName}/video/upload/so_1/${currentVideoPublicId}.jpg`;
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="relative w-full h-full overflow-hidden">
            <video
              ...
              onLoadedMetadata={(e) => setVideoDuration((e.target as HTMLVideoElement).duration || 30)}
        ...
        {/* Main Content Area */}
        {(() => {
          const showState = videoTime < 5
            ? "hidden"
            : videoTime >= (videoDuration - 10)
              ? "aside"
              : "visible";
            ...
            <>
              <div className="relative z-20 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-center min-h-screen pt-24 pb-28">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center w-full">
                  ...
                  <div className={`lg:col-span-8 flex flex-col items-center lg:items-start text-center lg:text-left transition-all duration-1000 ease-in-out ${showState === "hidden"
                    ...
                    {/* Badge */}
                    <div className="inline-flex items-center gap-2 px-4 py-2 mb-4 sm:mb-6 rounded-full bg-black/45 border border-[#8cc63f]/40 backdrop-blur-md shadow-lg">
                  ...
                  <div className="lg:col-span-4 flex flex-col items-center lg:items-end w-full mt-8 lg:mt-0">
                    <div
                      ref={statsRef}
                      className={`grid grid-cols-2 lg:grid-cols-1 lg:flex lg:flex-col gap-3 sm:gap-4 w-full max-w-md lg:max-w-[200px] transition-all duration-1000 ease-in-out ${showState === "hidden"
                        ? "opacity-0 translate-y-12 pointer-events-none"
                        : showState === "aside"
                          ? "opacity-0 translate-x-[120%] pointer-events-none"
                          : "opacity-100 translate-y-0 pointer-events-auto"
              ...
              <motion.a
                href="#explore"
                className={`absolute bottom-20 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1 text-white/50 hover:text-white transition-all duration-1000 ease-in-out group ${showState === "hidden" || showState === "aside"
              ...
              <div className={`absolute bottom-0 left-0 right-0 h-14 bg-[#0a0a14]/90 border-t border-white/10 flex items-center overflow-hidden z-30 pointer-events-auto transition-all duration-1000 ease-in-out ${showState === "hidden" || showState === "aside"
